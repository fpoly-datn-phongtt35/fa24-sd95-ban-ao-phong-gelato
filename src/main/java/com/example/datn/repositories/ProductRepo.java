...
//            "p.price as price " +
    ...
    @Query(value = "SELECT top(10) p.id, p.code, p.name, " +
            ...
            "COALESCE(SUM(bd.quantity),0) AS totalQuantity, COALESCE(SUM(bd.return_quantity),0) AS totalQuantityReturn, " +
            ...
            "group by p.id, p.code, p.name, brand.name, cat.name order by totalQuantity DESC",
            nativeQuery = true)
